...
class ArticulosObtenerArticulosPorRubroController extends Controller
{
    ...
    public function obtenerArticulosPorRubro($rubro, $clicod, $banderaVendedor, $cli_categoria)
    {
        try {
            $hostIMG = $this->determinarHostIMG(request()->getHost());

            $precioObtenido = ($banderaVendedor == 0) ? $this->clienteService->buscarCategoria($clicod) : "art_precmayo AS precio";

            $articulosPorRubro = collect($this->articuloService->obtenerTodosLosArticulosPorRubro($hostIMG, $precioObtenido, $cli_categoria, $rubro));

            $porcentajes = $this->clienteService->calcularPorcentajeCliente($clicod);
            $condVenta = $this->parametroService->obtenerCondVenta();
            if ($banderaVendedor == 0 && $condVenta && $condVenta->CondVenta == 1 && $porcentajes != 0) {
                $articuloCalculado = $this->calcularPrecio($clicod, $articulosPorRubro, $porcentajes);
                $articulo = $articuloCalculado;
            } else {
                $articulosPorRubro->each(function ($articulo) {
                    $articulo->precioInicial = $articulo->precio;
                    $articulo->precio = $this->calcularPrecioConDescuentos($articulo);
                });
                $articulo = $articulosPorRubro;
            }
