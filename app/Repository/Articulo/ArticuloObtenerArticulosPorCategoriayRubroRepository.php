...
class ArticuloObtenerArticulosPorCategoriayRubroRepository
{
	public function obtenerArticulosPorCategoriayRubro($hostIMG, $art_precio, $precioObtenido, $order_by, $params, $query, $paginacion)
	{
		$sql = "SELECT * FROM (SELECT art_codtex, art_codnum,
				...
				FROM (	 
					...
					AND AdicionalxArtic.ada_vigencia=1) 
					LEFT JOIN Adicional ON (AdicionalxArtic.ada_adicional = Adicional.adi_codigo) 
					WHERE art_vigencia = 1 AND art_carrito = 1 $query->tipoProducto $query->rubro AND art_preclista >= $params->precioMin AND art_preclista <= $params->precioMax
					) AS t WHERE Seq = 1 and numeroFilas between $paginacion->min and $paginacion->max";
